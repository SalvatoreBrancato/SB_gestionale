    ...
    return (
        <div className="bg-sky-50 h-full relative">
            ...
            {formDestroy && <div className="flex flex-col items-center justify-around absolute w-1/3 h-1/4 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-sky-100 border-2 border-sky-400 rounded-lg z-20">
                <span className="text-2xl px-2 font-t">Sei sicuro di volere eliminare il prodotto?</span>
            ...
            <div className="flex flex-col mb-10">
                {prodotto && <span className="text-3xl px-3 py-1 font-t">{prodotto.nome}</span>}
                {prodotto && <span className="text-xl px-3 py-1 font-s">{prodotto.descrizione}</span>}
                {prodotto && <span className="text-xl px-3 py-1 font-s">Prezzo d'acquisto: {prodotto.prezzoAcquisto}</span>}
                {prodotto && <span className="text-xl px-3 py-1 font-s">Prezzo di vendita: {prodotto.prezzoVendita}</span>}
                {prodotto && <span className="text-xl px-3 py-1 font-s">Listino: {prodotto.listino}</span>}
                {prodotto && <span className="text-xl px-3 py-1 font-s">Pezzi: {prodotto.pezzi}</span>}
                {prodotto && <span className="text-xl px-3 py-1 font-s">Note: {prodotto.note}</span>}
            </div>

            {prodotto.fattureAcquisti && prodotto.fattureAcquisti.length > 0 ? <div className="text-center font-t py-4">Fatture Acquisti</div> : <div className="text-center font-t py-4">Fatture Vendite</div>}
            ...
            <div className="w-full min-h-1/2 flex justify-around overflow-y-auto">
                ...
                {prodotto.fattureAcquisti && prodotto.fattureAcquisti.length > 0 &&
                    ...
                    <div className="w-1/2 h-full overflow-y-auto my-tab">
                        ...
                        <div className="p-2 flex text-center border-b-2 border-sky-200 mx-4 font-t">
                            <span className="w-1/5">Numero</span>
                            <span className="w-1/5">Data</span>
                            <span className="w-1/5">Fornitore</span>
                            <span className="w-1/5">Totale</span>
                            <span className="w-1/5">Note</span>
                        </div>
                        ...
                        {prodotto.fattureAcquisti.map((fattura, index) => {
                            return (
                                <div key={index} className="p-2 flex text-center border-b-2 hover:bg-sky-200 rounded-sm border-sky-200 mx-4 font-s">
                                    ...
                                    <span className="w-1/5">{fattura.data}</span>
                                    {
                                        prodotto.fornitore.map((elem, index) => {

                                            return (<span key={index} className="w-1/5">{elem.ragioneSociale}</span>)
                                        })
                                    }
                                    <span className="w-1/5">{fattura.totale}</span>
                                    <span className="w-1/5">{fattura.note}</span>
                                </div>
                            )
                        })}
                ...
                {prodotto.fattureVendita && prodotto.fattureVendita.length > 0 &&
                        ...
                        {prodotto.fattureVendita.map((fattura, index) => {
                            return (
                                <div key={index} className="p-2 flex text-center border-b-2 hover:bg-sky-200 rounded-sm border-sky-200 mx-4 font-s">
                                    ...
                                    <span className="w-1/5">{fattura.data}</span>
                                    {
                                        prodotto.cliente.map((elem, index) => {

...
export default function FattureAcqShowPage() {

    useEffect(dettaglioFatturaAcq, [])
    ...
    function dettaglioFatturaAcq() {
        axios.get(`http://localhost:3000/fattureAcquisti/${id}`)
            .then(response => {
                setFatturaAcq(response.data)
    ...
    function destroyFatturaAcq() {
        axios.delete(`http://localhost:3000/fattureAcquisti/${id}`)
            .then(response => {
    ...
    return (
        <div className='bg-sky-50 h-full relative'>
            ...
            {/* #########AGGIUNGERE FORM MODIFICA FATTURA############# */}
            ...
            <div className="flex justify-between m-3">
                ...
                <div>
                    <button onClick={() => eliminaFatturaAcq()}>
            ...
            <div className="flex flex-col mb-10">
                {fatturaAcq &&
                    <div className="w-full flex justify-between items-center px-5 mb-10">
                        <span className="text-3xl p-3 font-t">{fatturaAcq.fornitori.ragioneSociale}</span>
                        <span className="text-xl px-3 py-1 font-s">Fattura n. {fatturaAcq.numero} del {fatturaAcq.data}</span>
                    </div>
                }
                {fatturaAcq &&
                ...
                {fatturaAcq &&
                    <div className=" flex  w-2/3 mx-auto text-xl py-1 justify-end">
                        <div className="flex flex-col">
                            ...
                            <span className="me-2 my-2">Totale €</span>
                            <span className="me-2">Scadenza</span>
                        </div>
                        <div className="flex flex-col">
                            <span className="px-3 bg-white border-2 border-sky-200 rounded-lg text-right">{fatturaAcq.iva}%</span>
                            <span className="px-3 my-1 bg-white border-2 border-sky-200 rounded-lg text-right">{parseFloat(fatturaAcq.totale).toFixed(2)}</span>
                            {fatturaAcq.pagamento ? <span className="px-3 bg-white border-2 border-sky-200 rounded-lg text-right">{fatturaAcq.pagamento.scadenza}</span>:''}
                        </div>
                    </div>
                }
            </div>
